...
function storeMapper (stores) {
  ...
  const talkClient = stores.classifierStore.talkClient
...
class QuickTalkContainer extends React.Component {
  ...
  fetchComments () {
    ...
    this.props.talkClient.type('comments').get(query)
      .then (comments =>{
        ...
        this.props.talkClient.type('roles')
          .get({
            user_id: author_ids,
  ...
  async postComment (text) {
    ...
    try {
      ...
      const boards = await this.props.talkClient.type('boards').get({ section, subject_default: true })
      ...
      const discussions = await this.props.talkClient.type('discussions').get({
      ...
      if (existingDiscussion) { // Add to the existing discussion
        ...
        await this.props.talkClient.type('comments').create(comment).save()
        this.setState({
        ...
        await this.props.talkClient.type('discussions').create(discussion).save()
        this.setState({
          postCommentStatus: asyncStates.success,
          postCommentStatusMessage: '',
        })
